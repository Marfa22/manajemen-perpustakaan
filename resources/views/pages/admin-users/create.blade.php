...
@section('content')
  ...
  <div class="row">
    <div class="col">
      <form action="{{ request()->getBaseUrl() }}/admin/users" method="POST">
        @csrf
        <div class="card crud-form-card">
          <div class="card-header">
            <h3>Form Admin</h3>
            <p>Tambahkan akun admin dan atur hak akses menunya.</p>
          </div>
          <div class="card-body">
            <div class="crud-form-grid">
              ...
              <div class="form-group">
                ...
                <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="Opsional">
                @error('email')
                  <span class="invalid-feedback d-block">{{ $message }}</span>
                @enderror
              </div>
              ...
              <div class="form-group">
                ...
                <select name="role" id="role" class="custom-select @error('role') is-invalid @enderror" required>
                  <option value="{{ $roleMenuAdmin }}" {{ old('role', $roleMenuAdmin) === $roleMenuAdmin ? 'selected' : '' }}>Admin Menu</option>
                  <option value="{{ $roleSuperAdmin }}" {{ old('role') === $roleSuperAdmin ? 'selected' : '' }}>Super Admin</option>
                </select>
                @error('role')
                  <span class="invalid-feedback d-block">{{ $message }}</span>
                @enderror
                  ...
                  @foreach ($accessLabels as $accessKey => $accessLabel)
                    <label class="mb-0" style="display: inline-flex; align-items: center; gap: 6px;">
                      <input
                        ...
                        name="permissions[]"
                        value="{{ $accessKey }}"
                        {{ in_array($accessKey, old('permissions', []), true) ? 'checked' : '' }}
                      >
